...
class XORPerceptron:
    def __init__(self):
        # Perceptronではパラメータを変更したり活性化関数を変更してもXOR回路を再現できない
        self.weights_hidden_output = np.array([1, -2])         # 2x1の重み行列
        self.bias_output = 1.0                                 # 出力層のbias
...
if __name__ == "__main__":
    ...
    xor_nn = XORPerceptron()

    for i, input in enumerate(inputs):
        output = xor_nn.forward(input)
